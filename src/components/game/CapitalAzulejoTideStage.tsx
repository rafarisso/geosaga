...
import saoLuisStageBg from '../../assets/backgrounds/capital-sao-luis-stage-bg.png';
import baraoMareAltaBoss from '../../assets/bosses/capital-sao-luis-boss-barao-mare-alta.png';
import guindastePortoImg from '../../assets/enemies/capital-sao-luis-enemy-guindaste-porto.png';
import maresiaCorrosivaImg from '../../assets/enemies/capital-sao-luis-enemy-maresia-corrosiva.png';
import mareLixoImg from '../../assets/enemies/capital-sao-luis-enemy-mare-lixo.png';
import azulejoRachadoImg from '../../assets/enemies/capital-sao-luis-enemy-azulejo-rachado.png';
...
type AzulejoTideEnemyKind = 'azulejo' | 'lixo' | 'maresia' | 'porto';
type StepResult = 'playing' | 'quiz' | 'victory' | 'defeat';

type AzulejoTideEnemy = {
  id: number;
  kind: AzulejoTideEnemyKind;
  name: string;
  concept: string;
  x: number;
  lane: number;
...
type AzulejoTideMarker = { id: string; label: string; concept: string; x: number; lane: number; collected: boolean };
type AzulejoTideProjectile = { id: number; x: number; y: number; vx: number; damage: number; life: number };
type AzulejoTideParticle = { id: number; text: string; x: number; y: number; color: string; life: number; maxLife: number };

type AzulejoTideState = {
  guardian: RegionId;
  hp: number;
  maxHp: number;
  energy: number;
  ...
  bossAttack: number;
  bossShake: number;
  enemies: AzulejoTideEnemy[];
  markers: AzulejoTideMarker[];
  projectiles: AzulejoTideProjectile[];
  particles: AzulejoTideParticle[];
  quizCorrect: number;
  quizTotal: number;
  nextEnemy: number;
...
interface CapitalAzulejoTideStageProps {
...
const LANE_NAMES = ['Centro historico', 'Rampa da mare', 'Manguezal'] as const;
...
const ENEMY_DEFS = [
  { kind: 'azulejo', name: 'Azulejo Rachado', concept: 'Patrimonio fragil', x: 620, lane: 0, w: 122, h: 100, hp: 72, damage: 12, drift: 18, image: azulejoRachadoImg },
  { kind: 'maresia', name: 'Maresia Corrosiva', concept: 'Umidade salina', x: 1060, lane: 0, w: 132, h: 104, hp: 80, damage: 13, drift: 34, image: maresiaCorrosivaImg },
  { kind: 'lixo', name: 'Mare de Lixo', concept: 'Residuo costeiro', x: 1540, lane: 1, w: 148, h: 106, hp: 84, damage: 14, drift: 46, image: mareLixoImg },
  { kind: 'porto', name: 'Guindaste do Porto', concept: 'Pressao portuaria', x: 2100, lane: 2, w: 158, h: 116, hp: 94, damage: 16, drift: 12, image: guindastePortoImg },
] satisfies Array<Omit<AzulejoTideEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>>;

const MARKER_DEFS = [
  { id: 'centro-historico', label: 'Centro Historico', concept: 'Patrimonio', x: 430, lane: 0 },
  { id: 'azulejos', label: 'Azulejos', concept: 'Fachadas', x: 1120, lane: 0 },
  { id: 'baia-sao-marcos', label: 'Baia de Sao Marcos', concept: 'Mares fortes', x: 1840, lane: 1 },
  { id: 'manguezais', label: 'Manguezais', concept: 'Ilha costeira', x: 2420, lane: 2 },
];

const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

function createEnemies(): AzulejoTideEnemy[] {
  return ENEMY_DEFS.map((enemy, index) => ({ ...enemy, id: index + 1, maxHp: enemy.hp, passed: false, hitFlash: 0 }));
}

function createMarkers(): AzulejoTideMarker[] {
  return MARKER_DEFS.map((marker) => ({ ...marker, collected: false }));
}

function createState(guardian: RegionId): AzulejoTideState {
...
function cloneState(state: AzulejoTideState): AzulejoTideState {
  return {
    ...state,
...
function addParticle(state: AzulejoTideState, text: string, x: number, y: number, color = '#ffe26b') {
  state.particles.push({ id: state.nextParticle, text, x, y, color, life: 0.78, maxLife: 0.78 });
  state.nextParticle += 1;
}

function hurt(state: AzulejoTideState, amount: number, x: number, y: number) {
...
function rewardEnemy(state: AzulejoTideState, enemy: AzulejoTideEnemy, label: string) {
...
function damageEnemy(state: AzulejoTideState, enemy: AzulejoTideEnemy, amount: number) {
...
function damageBoss(state: AzulejoTideState, amount: number, canFinish: boolean) {
...
function fireShot(state: AzulejoTideState) {
...
function spawnBossWave(state: AzulejoTideState) {
  const variants: Array<Omit<AzulejoTideEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>> = [
    { kind: 'lixo', name: 'Mare de Lixo', concept: 'Baia pressionada', x: BOSS_X - 70, lane: 1, w: 148, h: 106, hp: 60, damage: 13, drift: 188, image: mareLixoImg },
    { kind: 'azulejo', name: 'Azulejo Rachado', concept: 'Patrimonio fragil', x: BOSS_X - 40, lane: 0, w: 122, h: 100, hp: 58, damage: 12, drift: 202, image: azulejoRachadoImg },
    { kind: 'porto', name: 'Guindaste do Porto', concept: 'Fluxo portuario', x: BOSS_X - 15, lane: 2, w: 154, h: 112, hp: 64, damage: 14, drift: 170, image: guindastePortoImg },
  ];
...
function processShots(state: AzulejoTideState, dt: number) {
...
function stepAzulejoTide(state: AzulejoTideState, input: InputState, dt: number): StepResult {
  ...
  const riskyLowTide = state.tideLevel < 0.22 && state.lane === 2;
  const riskyHighTide = state.tideLevel > 0.78 && state.lane === 1;
  if ((riskyLowTide || riskyHighTide) && state.tideDamageCooldown <= 0) {
    hurt(state, 7, state.px + 74, laneY(state.lane) - 100);
    addParticle(state, riskyLowTide ? 'lodo exposto' : 'mare invadiu', state.px + 84, laneY(state.lane) - 136, '#9ee9ff');
  ...
  if (!state.bossActive && (state.px > BOSS_X - 500 || (collectedAll && clearedBase))) {
    ...
    addParticle(state, 'Barao da Mare Alta', BOSS_X + 100, laneY(1) - BOSS_H - 12, '#ff786b');
  }
...
function playerStateFor(state: AzulejoTideState): AnimationState {
...
function starsFor(state: AzulejoTideState): number {
...
export function CapitalAzulejoTideStage({ mission, completed, onComplete }: CapitalAzulejoTideStageProps) {
  const [guardian, setGuardian] = useState<RegionId>('nordeste');
  const [snapshot, setSnapshot] = useState<AzulejoTideState>(() => createState('nordeste'));
  const stateRef = useRef<AzulejoTideState>(snapshot);
  ...
  function emitSfx(next: AzulejoTideState) {
  ...
  function handleStep(dt: number): boolean {
    const outcome = stepAzulejoTide(stateRef.current, inputRef.current, dt);
    if (outcome === 'quiz') {
  ...
  function answer(index: number) {
    ...
    if (correct) {
      ...
      addParticle(current, 'Azulejo Geo', current.px + 112, laneY(current.lane) - 122, '#ffe26b');
      playSound('special');
    } else {
  ...
  return (
    <section
      className="capital-play-root sao-luis-tide-root"
      style={{
        '--capital-stage-bg': `url(${saoLuisStageBg})`,
        '--region-color': character.themeColor,
      } as CSSProperties}
    >
      <div className={`capital-play-viewport sao-luis-tide-viewport ${state.specialFlash > 0 ? 'special-active' : ''}`} ref={viewportRef}>
        <div className="capital-play-scaler" style={worldStyle}>
          <div className="sao-luis-tide-world" style={{ width: STAGE_W, transform: `translateX(${-state.camera}px)` }}>
            <div className="sao-luis-tide-backdrop" style={{ width: STAGE_W }} />
            <div className="sao-luis-tide-glare" style={{ width: STAGE_W }} />
            <div className="sao-luis-tide-progress"><span style={{ width: `${progressRatio * 100}%` }} /></div>
            <div className="sao-luis-tide-tide" style={{ left: state.camera + 58 }}>
              <strong>{state.tideLevel > 0.72 ? 'Mare alta' : state.tideLevel < 0.28 ? 'Baixa-mar' : 'Mare virando'}</strong>
            </div>
            <div className="sao-luis-tide-lanes" style={{ width: STAGE_W }}>
              {LANES.map((y, index) => <div className={`sao-luis-tide-lane ${state.lane === index ? 'active' : ''}`} key={LANE_NAMES[index]} style={{ top: y - 28 }}><span>{LANE_NAMES[index]}</span></div>)}
            </div>
            <div className="sao-luis-tide-finish" style={{ left: BOSS_X + 36 }}><strong>Baia de Sao Marcos</strong><span>Finalize no quiz</span></div>

            {state.markers.map((marker) => (
              <div className={`sao-luis-tide-marker ${marker.collected ? 'collected' : ''}`} key={marker.id} style={{ left: marker.x, top: laneY(marker.lane) - 122 }}>
            ...
            {state.enemies.map((enemy) => {
              ...
              return (
                <div
                  className={`sao-luis-tide-enemy sao-luis-tide-enemy-${enemy.kind} ${enemy.passed ? 'passed' : ''}`}
                  key={enemy.id}
                ...
                >
                  ...
                  <div className="sao-luis-tide-enemy-health"><span style={{ width: `${ratio * 100}%` }} /></div>
            ...
            {state.bossActive && state.bossHp > 0 && (
              <div className="sao-luis-tide-boss" style={{ left: BOSS_X, top: laneY(1) - BOSS_H, width: BOSS_W, height: BOSS_H, transform: `translateX(${state.bossShake > 0 ? Math.sin(state.bossShake * 80) * 9 : 0}px)` }}>
                <img src={baraoMareAltaBoss} alt="" aria-hidden />
                <strong>Barao da Mare Alta</strong>
              </div>
            )}

            {state.projectiles.map((projectile) => <span className="sao-luis-tide-shot" key={projectile.id} style={{ left: projectile.x, top: projectile.y }} />)}

            <span className="sao-luis-tide-raft" style={{ left: state.px + 12, top: laneY(state.lane) - 26 }} />
            <Player
              region={guardian}
        ...
        <div className="sao-luis-tide-objective-panel">
          <strong>Mare dos Azulejos</strong>
          <span>Leia a mare, proteja os azulejos, colete marcos e finalize o boss com resposta correta.</span>
        </div>
      </div>

      <div className="capital-play-hud sao-luis-tide-hud">
        <div className="capital-hud-bars">
          <span className="capital-hud-name">{character.name} em Sao Luis</span>
      ...
      {needsRotate && (
        <div className="stage-rotate" role="alertdialog" aria-label="Gire o celular">
          <div className="stage-rotate-card">
            ...
            <p>A Mare dos Azulejos foi pensada para jogar na horizontal.</p>
      ...
      {phase === 'intro' && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-intro-card sao-luis-tide-intro">
            <span className="eyebrow">{completed ? 'Rejogar capital' : 'Nova fase jogavel'}</span>
            <h2>Sao Luis: Mare dos Azulejos</h2>
            <p className="stage-overlay-objective">Atravesse o Centro Historico, leia a mare da Baia de Sao Marcos, proteja os azulejos e derrote o Barao da Mare Alta apenas com conhecimento geografico.</p>
            ...
            <ul className="stage-controls-help">
              <li><strong>A/D</strong> ou <strong>setas</strong> controlam o guardiao entre ladeiras, rampa da mare e manguezal</li>
      ...
      {phase === 'quiz' && question && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-quiz-card">
            <span className="eyebrow">Quiz Geo de Sao Luis</span>
      ...
      {phase === 'victory' && (
        <div className="stage-overlay stage-overlay-victory">
          <div className="stage-overlay-card">
            ...
            <h2>Sao Luis concluida</h2>
            <div className="stage-stars" aria-label={`${stars} de 3 estrelas`}>{[1, 2, 3].map((n) => <span className={n <= stars ? 'star on' : 'star'} key={n}>*</span>)}</div>
            <p>Voce preservou os azulejos, leu a mare da ilha e conectou Centro Historico, manguezais e Baia de Sao Marcos.</p>
      ...
      {phase === 'defeat' && (
        <div className="stage-overlay stage-overlay-defeat">
          <div className="stage-overlay-card">
            <span className="eyebrow">A mare tomou conta</span>
            <h2>{character.name} foi vencido pela mare alta</h2>
            <p>Leia a mare, use os tiros geo contra os problemas da ilha e finalize o boss com uma resposta certa.</p>
