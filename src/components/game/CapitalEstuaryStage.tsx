...
import aracajuStageBg from '../../assets/backgrounds/capital-aracaju-stage-bg.png';
import senhorEstuarioBoss from '../../assets/bosses/capital-aracaju-boss-senhor-estuario.png';
import barcoAssoreadoImg from '../../assets/enemies/capital-aracaju-enemy-barco-assoreado.png';
import bueiroAlagadoImg from '../../assets/enemies/capital-aracaju-enemy-bueiro-alagado.png';
import calorOrlaImg from '../../assets/enemies/capital-aracaju-enemy-calor-orla.png';
import caranguejoPlasticoImg from '../../assets/enemies/capital-aracaju-enemy-caranguejo-plastico.png';
...
type EstuaryEnemyKind = 'caranguejo' | 'bueiro' | 'calor' | 'barco';
type StepResult = 'playing' | 'quiz' | 'victory' | 'defeat';

type EstuaryEnemy = {
  id: number;
  kind: EstuaryEnemyKind;
  name: string;
  concept: string;
  x: number;
  lane: number;
...
type EstuaryMarker = { id: string; label: string; concept: string; x: number; lane: number; collected: boolean };
type EstuaryProjectile = { id: number; x: number; y: number; vx: number; damage: number; life: number };
type EstuaryParticle = { id: number; text: string; x: number; y: number; color: string; life: number; maxLife: number };
...
type EstuaryState = {
  ...
  bossAttack: number;
  bossShake: number;
  enemies: EstuaryEnemy[];
  markers: EstuaryMarker[];
  projectiles: EstuaryProjectile[];
  particles: EstuaryParticle[];
  quizCorrect: number;
  quizTotal: number;
  nextEnemy: number;
...
interface CapitalEstuaryStageProps {
...
const LANE_NAMES = ['Ciclovia', 'Calcadao', 'Mangue'] as const;
...
const ENEMY_DEFS = [
  { kind: 'caranguejo', name: 'Caranguejo de Plastico', concept: 'Residuo na praia', x: 620, lane: 1, w: 118, h: 92, hp: 70, damage: 12, drift: 18, image: caranguejoPlasticoImg },
  { kind: 'bueiro', name: 'Bueiro Alagado', concept: 'Drenagem urbana', x: 1050, lane: 0, w: 124, h: 100, hp: 82, damage: 14, drift: 0, image: bueiroAlagadoImg },
  { kind: 'calor', name: 'Calor da Orla', concept: 'Ilha de calor', x: 1540, lane: 0, w: 122, h: 94, hp: 76, damage: 13, drift: 36, image: calorOrlaImg },
  { kind: 'barco', name: 'Barco Assoreado', concept: 'Estuario fragil', x: 2100, lane: 2, w: 136, h: 104, hp: 86, damage: 15, drift: 12, image: barcoAssoreadoImg },
] satisfies Array<Omit<EstuaryEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>>;
...
const MARKER_DEFS = [
  ...
  { id: 'mundau', label: 'Rio Sergipe', concept: 'Sistema lagunar', x: 2420, lane: 2 },
];

const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

function createEnemies(): EstuaryEnemy[] {
  return ENEMY_DEFS.map((enemy, index) => ({ ...enemy, id: index + 1, maxHp: enemy.hp, passed: false, hitFlash: 0 }));
}

function createMarkers(): EstuaryMarker[] {
  return MARKER_DEFS.map((marker) => ({ ...marker, collected: false }));
}

function createState(guardian: RegionId): EstuaryState {
...
function cloneState(state: EstuaryState): EstuaryState {
  return {
    ...state,
...
function addParticle(state: EstuaryState, text: string, x: number, y: number, color = '#ffe26b') {
  state.particles.push({ id: state.nextParticle, text, x, y, color, life: 0.78, maxLife: 0.78 });
  state.nextParticle += 1;
}

function hurt(state: EstuaryState, amount: number, x: number, y: number) {
...
function rewardEnemy(state: EstuaryState, enemy: EstuaryEnemy, label: string) {
...
function damageEnemy(state: EstuaryState, enemy: EstuaryEnemy, amount: number) {
...
function damageBoss(state: EstuaryState, amount: number, canFinish: boolean) {
...
function fireShot(state: EstuaryState) {
...
function spawnBossWave(state: EstuaryState) {
  const variants: Array<Omit<EstuaryEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>> = [
    { kind: 'caranguejo', name: 'Caranguejo de Plastico', concept: 'Praia pressionada', x: BOSS_X - 70, lane: 1, w: 120, h: 92, hp: 56, damage: 13, drift: 180, image: caranguejoPlasticoImg },
    { kind: 'bueiro', name: 'Bueiro Alagado', concept: 'Chuva e escoamento', x: BOSS_X - 40, lane: 0, w: 126, h: 98, hp: 58, damage: 12, drift: 198, image: bueiroAlagadoImg },
    { kind: 'barco', name: 'Barco Assoreado', concept: 'Banco de areia', x: BOSS_X - 15, lane: 2, w: 134, h: 102, hp: 60, damage: 13, drift: 168, image: barcoAssoreadoImg },
  ];
...
function processShots(state: EstuaryState, dt: number) {
...
function stepEstuary(state: EstuaryState, input: InputState, dt: number): StepResult {
  ...
  if ((riskyLowTide || riskyHighTide) && state.tideDamageCooldown <= 0) {
    ...
    addParticle(state, riskyLowTide ? 'areia quente' : 'mare forte', state.px + 84, laneY(state.lane) - 136, '#9ee9ff');
  ...
  if (!state.bossActive && (state.px > BOSS_X - 500 || (collectedAll && clearedBase))) {
    ...
    addParticle(state, 'Senhor do Estuario', BOSS_X + 100, laneY(1) - BOSS_H - 12, '#ff786b');
  }
...
function playerStateFor(state: EstuaryState): AnimationState {
...
function starsFor(state: EstuaryState): number {
...
export function CapitalEstuaryStage({ mission, completed, onComplete }: CapitalEstuaryStageProps) {
  const [guardian, setGuardian] = useState<RegionId>('nordeste');
  const [snapshot, setSnapshot] = useState<EstuaryState>(() => createState('nordeste'));
  const stateRef = useRef<EstuaryState>(snapshot);
  ...
  function emitSfx(next: EstuaryState) {
  ...
  function handleStep(dt: number): boolean {
    const outcome = stepEstuary(stateRef.current, inputRef.current, dt);
    if (outcome === 'quiz') {
  ...
  return (
    <section
      className="capital-play-root aracaju-estuary-root"
      style={{
        '--capital-stage-bg': `url(${aracajuStageBg})`,
        '--region-color': character.themeColor,
      } as CSSProperties}
    >
      <div className={`capital-play-viewport aracaju-estuary-viewport ${state.specialFlash > 0 ? 'special-active' : ''}`} ref={viewportRef}>
        <div className="capital-play-scaler" style={worldStyle}>
          <div className="aracaju-estuary-world" style={{ width: STAGE_W, transform: `translateX(${-state.camera}px)` }}>
            <div className="aracaju-estuary-backdrop" style={{ width: STAGE_W }} />
            <div className="aracaju-estuary-glare" style={{ width: STAGE_W }} />
            <div className="aracaju-estuary-progress"><span style={{ width: `${progressRatio * 100}%` }} /></div>
            <div className="aracaju-estuary-tide" style={{ left: state.camera + 58 }}>
              <strong>{state.tideLevel > 0.72 ? 'Mare alta' : state.tideLevel < 0.28 ? 'Seco quente' : 'Fluxo estavel'}</strong>
            </div>
            <div className="aracaju-estuary-lanes" style={{ width: STAGE_W }}>
              {LANES.map((y, index) => <div className={`aracaju-estuary-lane ${state.lane === index ? 'active' : ''}`} key={LANE_NAMES[index]} style={{ top: y - 28 }}><span>{LANE_NAMES[index]}</span></div>)}
            </div>
            <div className="aracaju-estuary-finish" style={{ left: BOSS_X + 36 }}><strong>Rio Sergipe</strong><span>Finalize no quiz</span></div>

            {state.markers.map((marker) => (
              <div className={`aracaju-estuary-marker ${marker.collected ? 'collected' : ''}`} key={marker.id} style={{ left: marker.x, top: laneY(marker.lane) - 122 }}>
            ...
            {state.enemies.map((enemy) => {
              ...
              return (
                <div
                  className={`aracaju-estuary-enemy aracaju-estuary-enemy-${enemy.kind} ${enemy.passed ? 'passed' : ''}`}
                  key={enemy.id}
                ...
                >
                  ...
                  <div className="aracaju-estuary-enemy-health"><span style={{ width: `${ratio * 100}%` }} /></div>
            ...
            {state.bossActive && state.bossHp > 0 && (
              <div className="aracaju-estuary-boss" style={{ left: BOSS_X, top: laneY(1) - BOSS_H, width: BOSS_W, height: BOSS_H, transform: `translateX(${state.bossShake > 0 ? Math.sin(state.bossShake * 80) * 9 : 0}px)` }}>
                <img src={senhorEstuarioBoss} alt="" aria-hidden />
                <strong>Senhor do Estuario</strong>
              </div>
            )}

            {state.projectiles.map((projectile) => <span className="aracaju-estuary-shot" key={projectile.id} style={{ left: projectile.x, top: projectile.y }} />)}

            <span className="aracaju-estuary-raft" style={{ left: state.px + 12, top: laneY(state.lane) - 26 }} />
            <Player
              region={guardian}
        ...
        <div className="aracaju-estuary-objective-panel">
          <strong>Patrulha da Orla e do Estuario</strong>
          <span>Troque de faixa, use tiros geo, colete marcos e finalize o boss com resposta correta.</span>
        </div>
      </div>

      <div className="capital-play-hud aracaju-estuary-hud">
        <div className="capital-hud-bars">
          <span className="capital-hud-name">{character.name} em Aracaju</span>
      ...
      {needsRotate && (
        <div className="stage-rotate" role="alertdialog" aria-label="Gire o celular">
          <div className="stage-rotate-card">
            ...
            <p>A patrulha de Aracaju foi pensada para jogar na horizontal.</p>
      ...
      {phase === 'intro' && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-intro-card aracaju-estuary-intro">
            <span className="eyebrow">{completed ? 'Rejogar capital' : 'Nova fase jogavel'}</span>
            <h2>Aracaju: Patrulha da Orla</h2>
            <p className="stage-overlay-objective">Patrulhe a Orla de Atalaia, mude de faixa no momento certo, proteja o manguezal e derrote o Senhor do Estuario apenas com conhecimento geografico.</p>
      ...
      {phase === 'quiz' && question && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-quiz-card">
            <span className="eyebrow">Quiz Geo de Aracaju</span>
      ...
      {phase === 'defeat' && (
        <div className="stage-overlay stage-overlay-defeat">
          <div className="stage-overlay-card">
            <span className="eyebrow">A patrulha perdeu o ritmo</span>
            <h2>{character.name} foi vencido pela mare</h2>
            <p>Troque de faixa cedo, atire nos problemas da orla, cuide do manguezal e finalize o boss com uma resposta certa.</p>
