...
import teresinaStageBg from '../../assets/backgrounds/capital-teresina-stage-bg.png';
import fornoMeioNorteBoss from '../../assets/bosses/capital-teresina-boss-forno-meio-norte.png';
import engarrafamentoPonteImg from '../../assets/enemies/capital-teresina-enemy-engarrafamento-ponte.png';
import asfaltoFerventeImg from '../../assets/enemies/capital-teresina-enemy-asfalto-fervente.png';
import ondaCalorImg from '../../assets/enemies/capital-teresina-enemy-onda-calor.png';
import mosquitoAlagadicoImg from '../../assets/enemies/capital-teresina-enemy-mosquito-alagadico.png';
...
type RiverHeatEnemyKind = 'mosquito' | 'asfalto' | 'calor' | 'ponte';
type StepResult = 'playing' | 'quiz' | 'victory' | 'defeat';

type RiverHeatEnemy = {
  id: number;
  kind: RiverHeatEnemyKind;
  name: string;
  concept: string;
  x: number;
  lane: number;
...
type RiverHeatMarker = { id: string; label: string; concept: string; x: number; lane: number; collected: boolean };
type RiverHeatProjectile = { id: number; x: number; y: number; vx: number; damage: number; life: number };
type RiverHeatParticle = { id: number; text: string; x: number; y: number; color: string; life: number; maxLife: number };
...
type RiverHeatState = {
  ...
  bossAttack: number;
  bossShake: number;
  enemies: RiverHeatEnemy[];
  markers: RiverHeatMarker[];
  projectiles: RiverHeatProjectile[];
  particles: RiverHeatParticle[];
  quizCorrect: number;
  quizTotal: number;
  nextEnemy: number;
...
interface CapitalRiverHeatStageProps {
...
const LANE_NAMES = ['Avenida quente', 'Ponte', 'Beira-rio'] as const;
...
const ENEMY_DEFS = [
  { kind: 'mosquito', name: 'Mosquito do Alagadico', concept: 'Agua parada', x: 620, lane: 2, w: 116, h: 94, hp: 68, damage: 12, drift: 34, image: mosquitoAlagadicoImg },
  { kind: 'asfalto', name: 'Asfalto Fervente', concept: 'Ilha de calor', x: 1050, lane: 0, w: 142, h: 92, hp: 84, damage: 15, drift: 6, image: asfaltoFerventeImg },
  { kind: 'calor', name: 'Onda de Calor', concept: 'Mormaco urbano', x: 1540, lane: 0, w: 126, h: 104, hp: 78, damage: 14, drift: 42, image: ondaCalorImg },
  { kind: 'ponte', name: 'Engarrafamento da Ponte', concept: 'Mobilidade travada', x: 2100, lane: 1, w: 156, h: 112, hp: 92, damage: 16, drift: 10, image: engarrafamentoPonteImg },
] satisfies Array<Omit<RiverHeatEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>>;

const MARKER_DEFS = [
  { id: 'ponte-estaiada', label: 'Ponte Estaiada', concept: 'Rio Poti', x: 430, lane: 1 },
  { id: 'parnaiba', label: 'Rio Parnaiba', concept: 'Eixo fluvial', x: 1120, lane: 2 },
  { id: 'poti', label: 'Rio Poti', concept: 'Confluencia', x: 1800, lane: 2 },
  { id: 'cidade-verde', label: 'Cidade Verde', concept: 'Sombra urbana', x: 2420, lane: 0 },
];

const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

function createEnemies(): RiverHeatEnemy[] {
  return ENEMY_DEFS.map((enemy, index) => ({ ...enemy, id: index + 1, maxHp: enemy.hp, passed: false, hitFlash: 0 }));
}

function createMarkers(): RiverHeatMarker[] {
  return MARKER_DEFS.map((marker) => ({ ...marker, collected: false }));
}

function createState(guardian: RegionId): RiverHeatState {
...
function cloneState(state: RiverHeatState): RiverHeatState {
  return {
    ...state,
...
function addParticle(state: RiverHeatState, text: string, x: number, y: number, color = '#ffe26b') {
  state.particles.push({ id: state.nextParticle, text, x, y, color, life: 0.78, maxLife: 0.78 });
  state.nextParticle += 1;
}

function hurt(state: RiverHeatState, amount: number, x: number, y: number) {
...
function rewardEnemy(state: RiverHeatState, enemy: RiverHeatEnemy, label: string) {
...
function damageEnemy(state: RiverHeatState, enemy: RiverHeatEnemy, amount: number) {
...
function damageBoss(state: RiverHeatState, amount: number, canFinish: boolean) {
...
function fireShot(state: RiverHeatState) {
...
function spawnBossWave(state: RiverHeatState) {
  const variants: Array<Omit<RiverHeatEnemy, 'id' | 'maxHp' | 'passed' | 'hitFlash'>> = [
    { kind: 'calor', name: 'Onda de Calor', concept: 'Ar seco', x: BOSS_X - 70, lane: 0, w: 126, h: 104, hp: 58, damage: 13, drift: 188, image: ondaCalorImg },
    { kind: 'mosquito', name: 'Mosquito do Alagadico', concept: 'Agua parada', x: BOSS_X - 40, lane: 2, w: 116, h: 94, hp: 56, damage: 12, drift: 202, image: mosquitoAlagadicoImg },
    { kind: 'ponte', name: 'Engarrafamento da Ponte', concept: 'Fluxo travado', x: BOSS_X - 15, lane: 1, w: 150, h: 108, hp: 62, damage: 14, drift: 170, image: engarrafamentoPonteImg },
  ];
...
function processShots(state: RiverHeatState, dt: number) {
...
function stepRiverHeat(state: RiverHeatState, input: InputState, dt: number): StepResult {
  ...
  const riverBreeze = state.lane === 2 ? 54 : state.lane === 1 ? 38 : 24;
  const heatDrag = state.lane === 0 ? state.tideLevel * 18 : state.tideLevel * 8;
  state.vx = clamp(state.vx, -120, topSpeed);
  state.px = clamp(state.px + (state.vx + riverBreeze - heatDrag) * dt, 18, STAGE_W - 170);

  const avenueOverheat = state.tideLevel > 0.66 && state.lane === 0;
  const bridgeOverheat = state.tideLevel > 0.84 && state.lane === 1;
  if ((avenueOverheat || bridgeOverheat) && state.tideDamageCooldown <= 0) {
    hurt(state, avenueOverheat ? 8 : 6, state.px + 74, laneY(state.lane) - 100);
    addParticle(state, avenueOverheat ? 'asfalto quente' : 'ponte abafada', state.px + 84, laneY(state.lane) - 136, '#ffbf69');
  ...
  if (!state.bossActive && (state.px > BOSS_X - 500 || (collectedAll && clearedBase))) {
    ...
    addParticle(state, 'Forno do Meio-Norte', BOSS_X + 100, laneY(1) - BOSS_H - 12, '#ff786b');
  }
...
function playerStateFor(state: RiverHeatState): AnimationState {
...
function starsFor(state: RiverHeatState): number {
...
export function CapitalRiverHeatStage({ mission, completed, onComplete }: CapitalRiverHeatStageProps) {
  const [guardian, setGuardian] = useState<RegionId>('nordeste');
  const [snapshot, setSnapshot] = useState<RiverHeatState>(() => createState('nordeste'));
  const stateRef = useRef<RiverHeatState>(snapshot);
  ...
  function emitSfx(next: RiverHeatState) {
  ...
  function handleStep(dt: number): boolean {
    const outcome = stepRiverHeat(stateRef.current, inputRef.current, dt);
    if (outcome === 'quiz') {
  ...
  function answer(index: number) {
    ...
    if (correct) {
      ...
      addParticle(current, 'Brisa Geo', current.px + 112, laneY(current.lane) - 122, '#ffe26b');
      playSound('special');
    } else {
  ...
  return (
    <section
      className="capital-play-root teresina-heat-root"
      style={{
        '--capital-stage-bg': `url(${teresinaStageBg})`,
        '--region-color': character.themeColor,
      } as CSSProperties}
    >
      <div className={`capital-play-viewport teresina-heat-viewport ${state.specialFlash > 0 ? 'special-active' : ''}`} ref={viewportRef}>
        <div className="capital-play-scaler" style={worldStyle}>
          <div className="teresina-heat-world" style={{ width: STAGE_W, transform: `translateX(${-state.camera}px)` }}>
            <div className="teresina-heat-backdrop" style={{ width: STAGE_W }} />
            <div className="teresina-heat-glare" style={{ width: STAGE_W }} />
            <div className="teresina-heat-progress"><span style={{ width: `${progressRatio * 100}%` }} /></div>
            <div className="teresina-heat-tide" style={{ left: state.camera + 58 }}>
              <strong>{state.tideLevel > 0.72 ? 'Calor extremo' : state.tideLevel < 0.28 ? 'Brisa do rio' : 'Calor moderado'}</strong>
            </div>
            <div className="teresina-heat-lanes" style={{ width: STAGE_W }}>
              {LANES.map((y, index) => <div className={`teresina-heat-lane ${state.lane === index ? 'active' : ''}`} key={LANE_NAMES[index]} style={{ top: y - 28 }}><span>{LANE_NAMES[index]}</span></div>)}
            </div>
            <div className="teresina-heat-finish" style={{ left: BOSS_X + 36 }}><strong>Encontro dos Rios</strong><span>Finalize no quiz</span></div>

            {state.markers.map((marker) => (
              <div className={`teresina-heat-marker ${marker.collected ? 'collected' : ''}`} key={marker.id} style={{ left: marker.x, top: laneY(marker.lane) - 122 }}>
            ...
            {state.enemies.map((enemy) => {
              ...
              return (
                <div
                  className={`teresina-heat-enemy teresina-heat-enemy-${enemy.kind} ${enemy.passed ? 'passed' : ''}`}
                  key={enemy.id}
                ...
                >
                  ...
                  <div className="teresina-heat-enemy-health"><span style={{ width: `${ratio * 100}%` }} /></div>
            ...
            {state.bossActive && state.bossHp > 0 && (
              <div className="teresina-heat-boss" style={{ left: BOSS_X, top: laneY(1) - BOSS_H, width: BOSS_W, height: BOSS_H, transform: `translateX(${state.bossShake > 0 ? Math.sin(state.bossShake * 80) * 9 : 0}px)` }}>
                <img src={fornoMeioNorteBoss} alt="" aria-hidden />
                <strong>Forno do Meio-Norte</strong>
              </div>
            )}

            {state.projectiles.map((projectile) => <span className="teresina-heat-shot" key={projectile.id} style={{ left: projectile.x, top: projectile.y }} />)}

            <span className="teresina-heat-raft" style={{ left: state.px + 12, top: laneY(state.lane) - 26 }} />
            <Player
              region={guardian}
        ...
        <div className="teresina-heat-objective-panel">
          <strong>Travessia dos Dois Rios</strong>
          <span>Troque de margem, busque sombra, use tiros geo e finalize o boss com resposta correta.</span>
        </div>
      </div>

      <div className="capital-play-hud teresina-heat-hud">
        <div className="capital-hud-bars">
          <span className="capital-hud-name">{character.name} em Teresina</span>
      ...
      {needsRotate && (
        <div className="stage-rotate" role="alertdialog" aria-label="Gire o celular">
          <div className="stage-rotate-card">
            ...
            <p>A travessia de Teresina foi pensada para jogar na horizontal.</p>
      ...
      {phase === 'intro' && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-intro-card teresina-heat-intro">
            <span className="eyebrow">{completed ? 'Rejogar capital' : 'Nova fase jogavel'}</span>
            <h2>Teresina: Travessia dos Dois Rios</h2>
            <p className="stage-overlay-objective">Atravesse a margem dos rios Parnaiba e Poti, mude de faixa no momento certo, proteja o parque ribeirinho e derrote o Forno do Meio-Norte apenas com conhecimento geografico.</p>
            ...
            <ul className="stage-controls-help">
              <li><strong>A/D</strong> ou <strong>setas</strong> controlam a travessia no eixo da margem</li>
              <li><strong>Espaco/W</strong> sobe de faixa; <strong>S</strong> desce de faixa</li>
              <li><strong>J</strong> atira; <strong>K</strong> abre o Quiz Geo para finalizar o boss</li>
            </ul>
            <button className="btn-primary btn-large" onClick={startMission}>Comecar travessia</button>
          </div>
        </div>
      )}
      ...
      {phase === 'quiz' && question && (
        <div className="stage-overlay">
          <div className="stage-overlay-card capital-quiz-card">
            <span className="eyebrow">Quiz Geo de Teresina</span>
      ...
      {phase === 'victory' && (
        <div className="stage-overlay stage-overlay-victory">
          <div className="stage-overlay-card">
            ...
            <h2>Teresina concluida</h2>
            <div className="stage-stars" aria-label={`${stars} de 3 estrelas`}>{[1, 2, 3].map((n) => <span className={n <= stars ? 'star on' : 'star'} key={n}>*</span>)}</div>
            <p>Voce leu o encontro dos rios, protegeu a beira-rio e conectou Ponte Estaiada, Rio Poti e Rio Parnaiba.</p>
      ...
      {phase === 'defeat' && (
        <div className="stage-overlay stage-overlay-defeat">
          <div className="stage-overlay-card">
            <span className="eyebrow">A travessia perdeu o ritmo</span>
            <h2>{character.name} foi vencido pelo calor</h2>
            <p>Use a beira-rio para aliviar o calor, atire nos problemas urbanos e finalize o boss com uma resposta certa.</p>
